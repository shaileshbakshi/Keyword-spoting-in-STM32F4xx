...
void init_plot()
{
  ...
  screen_size_x = BSP_LCD_GetXSize();
  screen_size_y = BSP_LCD_GetYSize();
...
void plot_mfcc()
{
  ...
  if(mfcc_update_counter==10) {
	  BSP_LCD_FillRect(0, screen_size_y/3, screen_size_x, screen_size_y/3);
    for (int i=0;i<10*kws->num_frames;i++) {
      for (int j=0;j<kws->num_mfcc_features;j++) {
        for (int x=0;x<x_step;x++) {
          for (int y=0;y<y_step;y++) {
        	  BSP_LCD_DrawPixel(x_start+i*x_step+x,200+j*y_step+y, mfcc_plot_buffer[i*kws->num_mfcc_features+j]);
          }
        }
      }
    }
...
void plot_waveform()
{
  ...
  BSP_LCD_FillRect(0,0,screen_size_x,screen_size_y/3);
